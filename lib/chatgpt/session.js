...
async function isChatGPTLoggedOut(page) {
  return page.evaluate(() => {
    ...
    const loginTexts = new Set(['Log in', 'Login', 'Sign in', '\u767b\u5165']);
    const signupTexts = new Set(['Sign up', 'Get started', '\u514d\u8cbb\u8a3b\u518a']);
    ...
    return [...document.querySelectorAll(loginSelectors)].some((element) => {
      ...
      return (
        ...
        loginTexts.has(text) ||
        loginTexts.has(aria) ||
        signupTexts.has(text) ||
        signupTexts.has(aria)
      );
    });
  });
}
...
async function assertChatGPTLoggedIn(page) {
  if (await isChatGPTLoggedOut(page)) {
    throw new Error(
      [
        'ChatGPT is not logged in in this work browser.',
        'Ask the user to sign in to ChatGPT in the opened work browser.',
        'After the user says they are logged in, rerun the ChatGPT image workflow with the same CDP URL.',
      ].join(' ')
    );
  }
}
