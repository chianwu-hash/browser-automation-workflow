...
async function hasImageMode(page) {
  return page.evaluate(({ imageNeedle, removeNeedle }) => {
    ...
    const imageAction = document.querySelector('[data-inline-selection-pill][data-keyword="創作圖像"], [data-inline-selection-pill][data-system-hint-type="picture_v2"]');
    if (imageAction && visible(imageAction)) {
      return true;
    }
...
async function ensureImageMode(page) {
  ...
  if (await clickCreateImageMenuItem(page)) {
    await page.waitForTimeout(1200);
  } else {
