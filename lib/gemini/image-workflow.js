...
async function runPromptSequence(page, prompts, options) {
  ...
  for (const entry of prompts) {
    ...
    const firstImage = page.locator('img.image.loaded, img[src^="blob:"]').first();
    if (await firstImage.isVisible().catch(() => false)) {
      await firstImage.hover({ force: true });
      await page.waitForTimeout(500);
      const downloadBtn = page.locator('button[data-test-id="download-generated-image-button"]').first();
    ...
    if (!downloaded) {
      ...
      await page.screenshot({ path: outputPath.replace('.jpeg', '.png'), fullPage: true });
    }
